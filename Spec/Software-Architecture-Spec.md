...
└── Software/                  # everything build-related lives under here — nothing software-side at the repo root
    ...
    └── Modules/
        ├── Bootloader/         # firmware image: bus-resident OTA bootloader, one binary for all boards
...
| Build system | CMake, one `CMakeLists.txt` per `Modules/*` producing a `.elf`, plus a top-level `Software/CMakeLists.txt` aggregating `Lib/*` and `Modules/*`. Configure from `Software/` (`cmake -S Software -B build`) |
...
| MCU target | STM32G031F8P6 (Cortex-M0+, 64 MHz, 64 KB flash / 8 KB SRAM, TSSOP20) for all four boards. HAL device define `STM32G031xx`. |
| Flashing | SEGGER J-Link. `add_stm32_executable()` adds a `flash-<name>` target per module that generates a commander script (`cmake/gen_jlink_script.cmake`) and runs `JLinkExe -device STM32G031F8`. |

The STM32 CMake toolchain file is `Software/cmake/arm-none-eabi-cortex-m0plus.cmake` (`-mcpu=cortex-m0plus -mthumb`, newlib-nano + `nosys`, `-fno-exceptions/-fno-rtti`). Startup + linker wiring: shared `Lib/Startup/startup_stm32g031xx.s` + `syscalls.c`, per-module `.ld` scripts, and `cmake/stm32.cmake` (`add_stm32_executable()` → `.elf` + `.bin`/`.hex` + `--print-memory-usage` + `flash-<name>`).

---

## 4. Code style

Same C++ style as `~/git/rollercoaster`, whose `Software/.clang-format` (copied verbatim) formats automatically. Conventions, not all of which `clang-format` enforces on its own:
...
- Constructors use member-initializer lists, one member per line, colon-aligned.
...
- `Node-Bus-Hardware-Design-Spec.md` §7: whether simultaneous full-stall across all 20 servos is a realistic scenario to design fuse/connector sizing around.
