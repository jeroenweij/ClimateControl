...
- **Source.** Every `LOG_*` macro goes through `Tools::Logger::Emit`, which records the line in `Tools::LogRing` (a fixed ring, `"<level>: <message>"` truncated to 32 characters and stamped with the MC's uptime in seconds; the MC's ring is sized per module, ~32 lines) and then calls the console sink `Logger::Write`, which on the MainController is empty unless the RTT option below is enabled.
...
- **Server.** The server keeps the most recent 500 `MainLog` lines in memory (not in SQLite), streams each to the browser over the websocket as it arrives, and serves the buffer at `GET /api/main/log`. The Node log panel on the Status page lists the MainController next to the bus nodes and shows it live, with no *Follow* polling.
- **When the uplink is down.** Only the ring (the last ~32 lines) holds the log, and it is read once the MC reconnects — enough to see why a bring-up attempt failed *if* it eventually connects. A unit that never connects is diagnosed with the RTT option or the bootloader-style LED codes.
- **RTT (bench backup, off by default).** The CMake option `CC_MC_LOG_RTT` (default `OFF`) compiles SEGGER RTT into `mainController` and makes `Logger::Write` write each line to RTT up-channel 0 in non-blocking (`NO_BLOCK_SKIP`) mode, so it can never stall the super-loop. It needs no pin, no UART and no extra hardware beyond the J-Link already used for flashing (`JLinkRTTClient` / `JLinkRTTLogger`), and costs ~0.5–1 KB of SRAM for the control block and up-buffer when enabled — nothing when off. It replaces the old bit-banged console (`Hal::BitBangSerial` on `Board::LogTx`, which busy-waited ~1 ms per character at 9600 baud and stalled the bus loop); with that gone `Board::LogTx` (PA5, the `ONEWIRE` net) is left unused on the MainController.
