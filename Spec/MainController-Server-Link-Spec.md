...
CRC-16/CCITT-FALSE over `NODE..DATA`, little-endian. `LEN` ≤ 35 (`MAX_DATA`, `RS485-Node-Protocol-Spec-STM32G030.md` §7/§9 — bumped from 32 with the v2 chunked-write redesign; this doc's old `≤ 32` was stale). TCP is a stream, so the server runs the same `Frame` deframer (SYNC scan → LEN → CRC) as a node; a sync loss just resynchronises. No length prefix or envelope is added.
...
7. **`targetNodeId == 0`** — MainController self-update, a different mechanism entirely, **not** an extension of steps 1-6. Not reachable via any relay (the MC doesn't relay to itself). Server → `OtaControl[Begin]` → `OtaData` chunks (each acked, same offset-resume semantics as the bus's `Firmware[Write]`) → `OtaControl[End]` (verifies) → `OtaControl[Activate]` (resets into the new app), all addressed to `NODE=0` on the uplink block itself (§5) — the running app takes no part; the target for all of this is MainController's *own dedicated bootloader* (`Software/Modules/MainBootloader/`), which dials the same uplink back out itself while resident, the same way the app does. The uplink drops when the app resets into the bootloader and reconnects (a fresh `UplinkHello`) once the bootloader's own `UplinkHandler` brings NINA back up; it drops again during `Activate`'s reset and reconnects once more on the new app.
...
3. **`ATO` data-mode relay** — the transparent byte pipe itself and the `UplinkHello` token handshake (§5) are not yet exercised end-to-end, only the Wi-Fi join and TCP peer connect underneath them (§3).
4. **`Roster` `Get` (on-demand refresh)** — `UplinkHandler::HandleUplinkFrame()` only answers a `Keepalive` `Get` from the server; a server-initiated `Get` on `Endpoint::Roster` is not handled at all. Today the server only ever receives a `Roster` once per uplink (re)connect, via `SendRoster()`'s unsolicited push — the "and on request" half of this doc's own §5 table entry is unimplemented.
5. **`OtaControl` / `OtaData` (`0x65`/`0x66`) have no server-side implementation.** MainController's own bootloader speaks both (§5, §8 step 7, `Software/Modules/MainBootloader/Firmware.cpp`), but `Webserver/internal/nodelib` has no Go encode/decode for either endpoint (nothing mirroring `firmwareproto.go`'s pattern for the bus `Firmware` protocol), and `Webserver/internal/service/ota.go` has no job type that drives the `Begin`/`OtaData`/`End`/`Activate` sequence against `NODE=0`. This path is untestable end-to-end until that's built, and MainController's own bootloader side hasn't been bench-verified against real hardware yet either.
6. **`CheckNodePresence()` / `SendRoster()` have no firmware-level test coverage** — `UplinkHandler`'s `NodeMaster`-derived roster/presence logic is only checked indirectly, via `NodeMasterTests.cpp`'s coverage of the `NodeActive`/`NodeInBootloader`/`NodeLastContactMs` getters it reads, plus a successful ARM build. `UplinkHandlerTests.cpp` has no fake at the NINA AT/data-mode level yet (its own standing scope note), so nothing exercises the actual diff-and-push behavior or the exact bytes `SendRoster()`/`CheckNodePresence()` put on the wire.
