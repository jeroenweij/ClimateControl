...
5. **`Roster` `Get` (on-demand refresh)** — `UplinkHandler::HandleUplinkFrame()` only answers a `Keepalive` `Get` from the server; a server-initiated `Get` on `Endpoint::Roster` is not handled at all. Today the server only ever receives a `Roster` once per uplink (re)connect, via `SendRoster()`'s unsolicited push — the "and on request" half of this doc's own §5 table entry is unimplemented.
6. **`OtaControl` / `OtaData` (`0x65`/`0x66`) are unimplemented** — defined in the endpoint table (§5) but nothing on either side constructs or handles them. Real OTA pushes today go through a different path entirely: the server drives them by relaying `Firmware` / `ThermostatFirmware` frames directly (the bus-native endpoints, not this uplink-only block) — see `Webserver/internal/service/ota.go`. Needs a decision: build these for real (and figure out what they'd do that the relay path doesn't already), or drop them from the endpoint table as superseded.
7. **`CheckNodePresence()` / `SendRoster()` have no firmware-level test coverage** — `UplinkHandler`'s `NodeMaster`-derived roster/presence logic is only checked indirectly, via `NodeMasterTests.cpp`'s coverage of the `NodeActive`/`NodeInBootloader`/`NodeLastContactMs` getters it reads, plus a successful ARM build. `UplinkHandlerTests.cpp` has no fake at the NINA AT/data-mode level yet (its own standing scope note), so nothing exercises the actual diff-and-push behavior or the exact bytes `SendRoster()`/`CheckNodePresence()` put on the wire.
