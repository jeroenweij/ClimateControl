...
5. **MainController self-update is not bench-verified end to end** (§8 step 7) — the bootloader (`Software/Modules/MainBootloader/`), the app-side `SystemControl` handling and the server driver are each built and unit-tested (`Firmware` host tests, a byte-level bootloader model in `Webserver/internal/service/ota_main_test.go`), but the full path against a real NINA module, Wi-Fi and server has not been run.
