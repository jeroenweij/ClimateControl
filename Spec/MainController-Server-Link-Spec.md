...
- **Events are judged by where they end up.** URCs arrive in bursts (IP up, down, up again), so each waiting state drains every queued event and decides on the end state: a network-down that was followed by a network-up does not fail the attempt, and before the IP is up only the timeout fails the wait. The peer closing (`+UUDPD`) is always final.
- **The network is given time to settle before the peer is opened.** After `+UUNU` the module is not yet ready for `AT+UDCP` — it answers `ERROR` until a second `+UUNU` fires, 0.3–0.9 s later on the bench. The link therefore waits for a 1 s stretch with no network event (any further one restarts the wait) before `AT+UDCP`, and re-sends a command that got `ERROR` or no reply only after 500 ms rather than back-to-back, so its three attempts span the window instead of finishing inside it. Network events seen before Wi-Fi is (re)activated belong to the module's previous life — one can still arrive just after the reset line is pulled — and are discarded, so they cannot stand in for the new connection's own `+UUNU`.
- **Liveness in data mode is a decoded frame, not a byte, and a keepalive must be answered.** Once the peer is gone the module is back in command mode and can answer stray writes with text (`ERROR`, `+UUDPD`); those bytes must not keep a dead link alive. The MC sends a `Keepalive` `Get` every 10 s and the server answers each with a `Keepalive` `Report`. Only that `Report` counts as the answer: other inbound frames (an OTA transfer's writes, say) prove only that the server can still reach the MC, which stays true when the module has stopped forwarding the MC's bytes. If no `Report` arrives within 10 s of a keepalive the link is treated as dead (a dropped peer, a half-open connection, a one-way stall, or a server that is up but not answering) and the NINA is reset and re-joined — within 20 s at worst, whatever the module does or doesn't signal. A 60 s stretch with no valid frame at all is a backstop behind that.
- **Every session opens with the `UplinkHello`.** The server rejects a connection whose first frame is anything else. When a session starts, whatever was staged for the previous one (a keepalive queued as its deadline expired, frames from the moment data mode was entered) is dropped and the hello is queued first, followed by the roster.
...
| `0x64` `Keepalive` | ↔ | `Get` / `Report` | none | Idle liveness, 10 s interval. A keepalive not answered (by any valid frame) within 10 s triggers reconnect (§3). |
...
| `0x68` `MainLog` | MC→S | `Report` | `uptimeSec(3 LE) · text(≤32)` | One line of MainController's log, pushed unsolicited (§5.1). |
...
### 5.1 MainController log (`MainLog`)

MainController has no hardware debug console (both USARTs are committed, `MainController-Spec.md` §5), so its log reaches the operator over the uplink instead — the same lines a bus node exposes through `DiagLog` (`Node-Message-Model-Spec.md` §3), but *pushed*, because the MC (unlike a bus node) has an always-on link to the server and nobody has to poll for it.

- **Source.** Every `LOG_*` macro goes through `Tools::Logger::Emit`, which records the line in `Tools::LogRing` — the same fixed ring every bus node has: 10 lines, each `"<level>: <message>"` truncated to 32 characters and stamped with the MC's uptime in seconds and then calls the console sink `Logger::Write`, which on the MainController is empty unless the RTT option below is enabled.
- **Push.** Once the uplink is in data mode, `UplinkHandler` moves lines from the ring into its outbound queue as `MainLog` `Report`s, at most two per super-loop pass and only while the queue is under half full, so log traffic never displaces relayed bus frames. The drain path never logs itself. Lines logged before the link is up (Wi-Fi/NINA bring-up, boot) wait in the ring and flush right after the `UplinkHello`; if the ring overflowed meanwhile the first line is `~ <n> lost`.
- **Time.** The frame carries the MC's uptime, not wall-clock time. The server places a line at `helloWallTime − helloUptime + lineUptime`, using the `uptimeSec` in the `UplinkHello` that opened the connection.
- **Server.** The server keeps the most recent 500 `MainLog` lines in memory (not in SQLite), streams each to the browser over the websocket as it arrives, and serves the buffer at `GET /api/main/log`. The **Logs** page lists the MainController next to the bus nodes and shows its log live, with no *Follow* polling.
- **When the uplink is down.** Only the ring (the last 10 lines) holds the log, and it is read once the MC reconnects — enough to see why a bring-up attempt failed *if* it eventually connects. A unit that never connects is diagnosed with the RTT option or the bootloader-style LED codes.
...
4. Server streams `Set Firmware[Write] {offset(2 LE), bytes=32}` with 4 chunks in flight (go-back-N, `otaNodeWindow`), each answered by that write's own `Ack`/`Nack` (echoing the offset + a flash read-back CRC16) rather than a generic `Firmware Status` Report — the reply is tied to the specific write (`Node-Flash-Layout-and-Bootloader-Spec.md` §6.2). Acks are cumulative; a `Nack` names the offset to resync to; no progress for the write timeout resends from the oldest unacked chunk. Keeping several chunks in flight keeps the uplink busy — one chunk per ~140 ms round trip is the cadence at which the NINA stops forwarding (§11 item 3) — and roughly quadruples throughput (~300 B/s on the bench). A Thermostat push stays at one chunk in flight: its ControllerNode relays one at a time. Delivery still rides the target's normal poll window.
...
6a. **A bus push survives the uplink going quiet.** The bus node (or the Thermostat behind a ControllerNode) is untouched when the module stalls and the MainController resets its NINA, so it keeps everything it has received. When a chunk stays unanswered through its retries the server waits for the uplink to return (up to 120 s), probes `Firmware[Get]` (`ThermostatFirmware[Get]` for a thermostat push) and continues from the `expectedOffset` the bootloader reports. A bootloader holding no transfer (`Idle`: it restarted) is sent `Begin` again and the image starts over; a node that answers nothing for 90 probes is presumed to have booted its application and is sent `EnterBootloader` and `Begin` again (not for a thermostat push, whose `Begin` does that CN-side). Bus pushes send one chunk at a time; acks ride the bus poll cadence, so the window used for the MainController's own update does not apply.
7. **`targetNodeId == 0`** — MainController self-update, a different mechanism entirely, **not** an extension of steps 1-6. Not reachable via any relay (the MC doesn't relay to itself). The server first parks the running app in its bootloader with `SystemControl[Set] {2}` addressed to `NODE=0` — the app's `UplinkHandler` handles a `NODE=0` `SystemControl` itself instead of relaying it onto the bus; it Acks, sets the backup-register magic and resets. The bootloader (`Software/Modules/MainBootloader/`) dials the uplink back out itself, sends a fresh `UplinkHello`, and answers `OtaControl[Get]` with a status `Report` — the server polls for that (any report proves the reset landed; the running app ignores `OtaControl`). Then server → `OtaControl[Begin]` → `OtaData` chunks (each acked, same offset-resume semantics as the bus's `Firmware[Write]`; up to 4 in flight at once, go-back-N: an ack for a chunk also covers every chunk before it, a `Nack` names the offset to restart from — repeats of it are ignored briefly, since one loss draws a `Nack` from each chunk sent behind it — and no ack progress for 3 s resends from the oldest unacked chunk) → `OtaControl[End]` (verifies) → `OtaControl[Activate]` (resets into the new app), all addressed to `NODE=0` on the uplink block itself (§5). The uplink drops when the app resets into the bootloader and again during `Activate`'s reset, reconnecting each time with a fresh `UplinkHello` (the bootloader's reports `fwVersion = 0`). A transfer also survives the link going quiet: the module can stop forwarding the MC's bytes for tens of seconds, after which the MC resets the NINA and reconnects while its bootloader keeps everything received so far. When a chunk stays unanswered through its retries the server waits for the uplink to return (up to 120 s), probes `OtaControl[Get]`, and continues from the `expectedOffset` it reports; a bootloader that has lost its state (`Idle`) is sent `Begin` again and the image starts over. An ack is matched to the chunk by offset, so a late ack for an earlier chunk is ignored. The server driver is `Webserver/internal/service/ota_main.go`.
...
| `MainLog` drain (§5.1) + the MC-sized log ring | ~0.3 KB | ~1.1 KB (32 × 32 B lines + stamps) |
| **Total** | **~4–6 KB** | **< 1 KB** |
...
3. **The NINA stops forwarding UART bytes to TCP now and then** — reproduced on the bare module with no MainController (a PC as server, an adapter on the module's TXD/RXD): the module keeps acknowledging what the server sends and delivers it to the UART, but stops passing what arrives on its UART to the peer, for seconds to minutes until it is reset. It shows up when traffic crosses the module with gaps of about 120 ms or more (request periods of 130–250 ms stalled; 30–110 ms did not in the runs made), which is the cadence of a stop-and-wait OTA transfer. UART flow control and Wi-Fi power save are not the cause. The link copes (an unanswered keepalive resets the NINA within 20 s, and the transfer resumes, §3, §8), but each stall costs a retry or a reconnect. The OTA transfer keeps several chunks in flight (§8 step 7), so the link stays busy and the cadence is avoided; the module's own trigger is not identified, so other traffic patterns with quiet stretches can still hit it. A second failure mode shows up around those resets: the module sometimes boots (`+STARTUP`) and then answers no `AT` at all; only another reset brings it back. `NinaLink` therefore gives the post-reset `AT` probe ~20 s (10 attempts) before resetting the module again, rather than probing indefinitely — unbounded, one such boot kept the uplink down for 7½ minutes.
