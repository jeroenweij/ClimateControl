...
The link uses the same `THVD2410DR` (LCSC `C1849398`) as the main bus, so the same passives apply — but simpler, because this link's topology never varies: it is always exactly `ControllerNode` at one end and `Thermostat` at the other, so there's no need for the main bus's DNP-plus-jumper "populate only wherever the physical ends happen to be" scheme. Populate directly:
...
| Fail-safe bias | A→3V3, B→GND, ~560 Ω each | **once for the whole link, at ControllerNode** (the link's master, `LinkMaster`, §5.3) — not at the Thermostat, and not both: a second bias network would just parallel down and skew the idle point for no benefit. The THVD2410 receiver is itself fail-safe (logic high with the inputs open or shorted), so the bias is belt-and-braces for the idle level rather than required. `R20`/`R21`, `0603WAF5600T5E`, LCSC `C23204` (Basic part) — R20 A→3V3, R21 B→GND, confirmed on the CN-side link net in the netlist. Actual current draw is ~2.8mA (3.3V across the 560+560Ω pair plus the ~60Ω of parallel A-B termination at both ends) — negligible against the rail. |
...
| ESD/surge | SM712 RS-485 TVS, SOT-23-3, pin 1→A, pin 2→B, pin 3→GND | both ends, right at the connector — LCSC `C5199207` (ElecSuper) or `C404012` (genuine Bourns). Unlike the main bus (where any A/B clamp below ~70V is DNP, `Node-Bus-Hardware-Design-Spec.md` §6.3), the highest voltage on this cable is the +5V feed, inside the SM712's −7/+12V non-conducting window — so a hot-plug or ground-last mating can't drive it into conduction, and it adds external ESD margin on top of the THVD2410's own. The Thermostat's connector sits at an exposed wall location, if anything more ESD-exposed than an enclosed duct node. |
...
The server makes the same check *before* creating the job — it has the Thermostat's version from `0x63` and the target from the held image — and refuses it (`409`, "already current") rather than starting a pointless transfer (`service.EnqueueUpdate`). The CN guard is the backstop for a stale server cache. `Force` is set only on an explicit operator "re-flash anyway": `"force": true` on `POST /api/firmware/update`, or the Firmware page's allow-downgrade / same-version toggle. It is stored on the job (`ota_jobs.force`), so a queued push still carries it into `Begin`. Match is on `fwVersion` only (the cached `SystemInfo` carries no `buildId`).
...
- **Presence / version / identity:** uplink endpoint `0x63 ThermostatStatus`, MC→S `Report {controllerNodeId(1), linkUp(1), blState(1), fwMajor(1), fwMinor(1), uid[12]}` (17 B), on change, for every known Thermostat on each uplink (re)connect, and as a 60 s keepalive. The MC (`ThermostatMonitor`) fills it from each ControllerNode's `RoomLink` and `ThermostatFirmware` Status `Report`s — watched as they pass, and asked for with a `Get` pair, one ControllerNode every 2 s so the bus queue never sees a burst. A ControllerNode whose Thermostat is in its bootloader is not polled: the running push's own Status `Report`s keep the picture current. `blState` is the Status `state` byte (`0` = application).
  - **`uid`** is sent as zeros for now: the CN does not read or cache its Thermostat's MCU ID yet (open item below). It is meant to be the Thermostat MCU's 96-bit factory device ID (read-only at `0x1FFF_7590`, `Node-Flash-Layout-and-Bootloader-Spec.md` §2). The CN reads it once from the Thermostat's `SystemInfo` (`uid[12]`) over the link and caches it. Same role as `nodes.uid` for bus nodes: it names the physical unit independent of the assigned `nodeId`, so the server can tell a Thermostat behind a given CN was physically swapped (new `uid`, same id). Inventory / OTA-history signal, not used for routing.
- **OTA target:** the server drives `ThermostatFirmware` directly, addressed to the owning ControllerNode's node id, instead of `Firmware` addressed to a plain node — same relayed-endpoint mechanism, different endpoint (`MainController-Server-Link-Spec.md` §8 step 6). No MC-side translation is involved; `ThermostatFirmware` is itself an ordinary relayed endpoint (§5.4 table above).
...
Built in the MainController: `ThermostatMonitor` emitting `0x63 ThermostatStatus` (§5.6). A Thermostat push needs nothing else from the MC — `ThermostatFirmware` is an ordinary relayed endpoint, addressed to the owning ControllerNode (§5.6 "OTA target"). Built in the server: the pre-flight already-current check and `Force` re-flash (§5.4.1).

Not built yet: the Thermostat `uid` in `0x63` — the CN would read it from the Thermostat's `SystemInfo` and cache it, but node `SystemInfo` carries no UID today, so it is sent as zeros.
