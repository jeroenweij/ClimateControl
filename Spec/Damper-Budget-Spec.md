...
void RoomControlLoop::SetBudget(const uint8_t percent)
{
    ...
    {
        damper.SetTarget(budget);  // re-clamp immediately, not just on the next Loop() tick -- gated on
                                    // Auto so a technician's Manual override is never silently reclamped
    }
}
```
