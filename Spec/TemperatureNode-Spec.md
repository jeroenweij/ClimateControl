...
2. **Update/report rate** — how often does a temperature reading need to change the bus state? Duct air temperature changes slowly compared to a digital input — the existing debounce-on-change pattern may need a minimum report interval added on top, not just change-detection, so `MainController` doesn't conclude the node is dead during a long stretch of unchanged readings (though the heartbeat/poll cycle already covers liveness independent of value changes).
