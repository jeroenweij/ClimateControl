...
The shared power rail runs at 48V DC, stepped down locally at each node to 5V (servo) and 3.3V (MCU).
...
| 3, 6 | Green | **ENABLE** (bonded, both wires) | One control line — no separate `NRST` bus (see below). Both green conductors bonded together for ENABLE, same halved-resistance rationale as the V+/GND pairs. |
...
**Drive scheme: one pull-up per node, one pull-down at the master.** Each node pulls the ENABLE net up to its *own local 48V rail* through a high-value resistor. The master has a single low-side N-MOSFET that pulls the whole net to GND to disable every node at once. There is no pull-up at the master. A node holds itself enabled from its own 48V and GND, so the shared line — and any IR drop or ground offset along it — never sets the "enabled" level; only the disable edge travels the wire, carrying just the ~1mA total pull-up current, so drop along the ENABLE conductor is a few mV and irrelevant.
...
**Receiving end:** the LMR16030 EN pin tolerates 0–60V with a ~1.2V threshold (datasheet §6.1/§6.5), so no protection is needed. A per-node RC + Schmitt filter is cheap transient-noise insurance but not load-bearing.
...
| All 20 servos stalled simultaneously (worst case) | 20 × 2.7A ≈ 54 A | **`(54A × 5V)/(48V × 0.875) ≈ 6.4A`** |

Split across the both-ends power injection (mitigation below) that's **~3.2A at each injection point** — at or just past the top of the "~2–3A safe headroom per hop" a bonded-pair connector gives.

**Decision:**
- Normal operation (~1.4A) is comfortably within a bonded-pair connector's safe range — no special handling needed for the typical case. The servo is per-node power-gated (`Node-Bus-Power-Path-Spec.md` §3.1), so both rows above are *transients* bounded by how many dampers move at once; the resting bus current is ~20 × (MCU + transceiver) ≈ tens of mA. The both-ends feed below is headroom for a coordinated multi-damper move.
- Worst-case simultaneous full-stall (~6.4A) can exceed the ~2–3A single-hop connector budget.
- **Mitigation: power injection at both ends of the chain**, not multi-point injection every 4–5 nodes. With the bus length ~100m, feeding both ends splits each node's return current toward whichever end is electrically closer, cutting the worst-case IR drop to roughly 1/4 of the single-end-fed case — see §6 for the RS485 common-mode margin this resolves. Requires an actual physical power connection at the far end of the run (a second supply there, or a dedicated feed cable back to the master). RS485 stays a single continuous bus end-to-end regardless — only the power pairs need the second feed point.
...
**Hong Cheng HC-WK88-H18-B — LCSC/JLCPCB part C7501838**

| Spec | Value |
|---|---|
| Type | 8P8C (RJ45), single port |
| Magnetics | **None** — plain 8-pin pass-through, no transformer/coil circuitry (a magnetics/transformer jack would block DC power and distort RS485) |
| Shielding | Yes, metal shell (stainless steel) |
| LED | None |
| Mount | Through-hole, right-angle |
| Current rating | 1.5A per contact — matches the connector current budget §4's calculations are based on |
...
**Fallback:** C7501830 (Hong Cheng HC-WK88-H132-55B) — same electrical spec, a different housing variant in the same series, not footprint-interchangeable with C7501838.

Tie the shield to chassis/earth ground rather than signal ground if a separate protective earth is available.
...
**MCU: STM32G031F8P6.** TSSOP20, 64 MHz Cortex-M0+, 8 KB SRAM, 64 KB flash (room for a bus-resident DFU bootloader + the MainController's NINA-W152 driver), `LPUART1`, `RTC` + backup registers (a wear-free "enter bootloader" flag, and an optional wall-clock if an LSE crystal is fitted), `TIM2` (32-bit), 7 DMA channels. HAL device define `STM32G031xx`. Footprint stays compatible with STM32G031F6P6 (32 KB) as a cost-down fallback.
...
| Clock | **HSI16 only** | HSI16 is ±0.75% trimmed, ±1%/0–85°C, ±2% at −40°C — fine for the 115200 baud bus: an indoor duct/wall sees ~10–40°C where HSI16 holds ≈±1%, so two nodes are ~±2% apart, inside the async-UART framing budget. 115200 is not an exact integer divisor of the HSI16-derived USART clock (16 MHz/115200 ≈ 138.9 → BRR rounds to 139, actual baud ≈115108 Hz, ≈0.08% generator error) — negligible next to the HSI16 spread itself. This is why 1 Mbit is not used. Pins 2/3 (`PB9/PC14-OSC32_IN`, `PC15-OSC32_OUT`) can instead take a **32.768 kHz LSE crystal** for the G031 RTC (wall-clock for MainController schedules). On the MainController board `PB9` is the bus-disable output (`RESET_NODES`), so LSE there would need it relocated; on Node/Thermostat boards pins 2/3 are free. Footprint LSE **DNP** on all boards for now. |
...
USART1 on port B + PA12, no SYSCFG pad remap needed:
...
|---|---|---|---|---|
| 7  PA0  | USART2_CTS (AF1) ← NINA RTS | **servo stall-current sense** (`ADC_IN0`, `Node-Bus-Power-Path-Spec.md` §3.1) | — | — |
...
6. **Test points:** 48V, 5V, 3V3, GND, A, B, debug-UART TX, SWD header.
...
1. **Whether simultaneous full-stall across all 20 nodes is a realistic scenario** for this application (e.g. synchronized power-on homing) or a non-issue because servos move independently. Relevant for fuse/PTC and connector-current sizing (§4).
