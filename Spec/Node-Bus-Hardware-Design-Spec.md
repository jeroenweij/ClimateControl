...
| Clock | **HSI16 only** | HSI16 is ±0.75 % trimmed, ±1 %/0–85 °C, ±2 % at −40 °C — fine for the **115 200 baud** bus (§7 item 3): an indoor duct/wall sees ~10–40 °C where HSI16 holds ≈ ±1 %, so two nodes are ~±2 % apart, inside the async-UART framing budget. 115 200 is **not** an exact integer divisor of the HSI16-derived USART clock (16 MHz/115 200 ≈ 138.9 → BRR rounds to 139, actual baud ≈115 108 Hz, ≈0.08 % generator error) — negligible next to the HSI16 spread itself, so the framing-budget conclusion still holds, just not for a "zero generator error" reason. This is why 1 Mbit is not used. Pins 2/3 (`PB9/PC14-OSC32_IN`, `PC15-OSC32_OUT`) can instead take a **32.768 kHz LSE crystal** for the G031 RTC (wall-clock for MainController schedules). On the MainController board `PB9` is the bus-disable output (`RESET_NODES`), so LSE there would need it relocated; on Node/Thermostat boards pins 2/3 are free. Footprint LSE **DNP** on all boards for now. |
...
3. ~~**Cable run length per segment / RS485 baud rate**~~ — total bus length ~100 m. Not a factor for ENABLE (§3's per-node pull-up scheme removes the ground-offset exposure entirely). **Baud: 115 200** (`RS485-Node-Protocol-Spec-STM32G030.md` §9) — not an exact integer USART divisor (≈0.08 % generator error at 16 MHz HCLK, negligible next to the HSI16 clock spread that already spends most of the async-UART budget); length·rate = 1.15×10⁷ bit·m/s sits deep inside the safe region for 100 m of terminated twisted pair with 20 lightly-loaded stubs. 250 000 / 500 000 (both exact) are headroom steps only if bench-validated on the real cable; 1 Mbit is not recommended — too little margin against HSI16 spread + loaded edges for a duct-buried bus.
