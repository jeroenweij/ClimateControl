...
4. **`MainBootloader` (§7.1) is not yet bench-verified** — the bootloader, the app-side `SystemControl` handling and the server driver (`Webserver/internal/service/ota_main.go`) are built and unit-tested, but the full path — app parks itself, bootloader joins Wi-Fi and reconnects, image streams and activates — has not been run on hardware against a real NINA module and server. The bootloader has no logging (`Tools::Logger` is not linked to stay inside the 10 KB budget); only the error LED reports faults.
