...
**Flow:** `Begin` → wait for its `Ack`/`Nack` → stream `Write` frames with up to 4 in flight (go-back-N), each answered by its own `Ack`/`Nack` → an `Ack` for offset X confirms every chunk up to X (the node takes chunks strictly in order), a `Nack` names the offset to resync to, and no progress for the write timeout resends from the oldest unacked chunk → repeat to `imageSize` → `End` → wait for its `Ack`/`Nack` → `Activate`. `Begin`/`End`/`Abort` reuse the same correlation via `Ack`/`Nack` too, for consistency with `Write` — the reply is a single `lastError` byte (`0` on `Ack`, one of `Lib/NodeLib/EFirmware.h`'s `FirmwareError` codes on `Nack`), since none of the three has anything else to report. The master still keeps a `Status`-report probe (`Get` on `Endpoint::Firmware`, unconditionally re-armed) alongside this on the same timeout budget, to recover from a lost `Set` or a lost `Ack`/`Nack` — the `Status` report's `state`/`lastError` say the same thing the op's own reply would have.
...
**Window on the node side.** The MainController relays the in-flight `Write`s onto the bus back to back, so several can arrive between two `Poll`s. `Boot::FirmwareSlave` stages them in RAM (up to 8) and programs them, in order, only when its `Poll` arrives: programming flash stalls the CPU (the code runs from flash), and programming each chunk as it arrived lost bytes of the next one to a USART overrun. The master is quiet from its `Poll` until the node's `Done`, so nothing arrives while flash is being written. Each staged `Write` gets its own `Ack`/`Nack`, queued (up to 8, oldest dropped first) and sent in order on that same `Poll`, ahead of `Done`. `End` commits anything still staged first.
