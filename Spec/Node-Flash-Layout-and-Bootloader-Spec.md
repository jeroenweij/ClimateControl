...
**Flow:** `Begin` → poll until `state==bl-receiving` → stream `Write` frames, each waiting for its own `Ack`/`Nack` → on a lost ack, resend the same `Write`; on a `Nack`, resync to the offset it names → repeat to `imageSize` → `End` → poll for `bl-valid` → `Activate`. `Begin`/`End`/`Abort` reuse the same correlation via `Ack`/`Nack` too, for consistency with `Write`.
