...
- **On only during a move.** Firmware sets the PWM (50 Hz on TIM3_CH1, 500–2500 µs for 0–100 %, `Hal::Pwm`) to the new target, asserts `ServoEnable`, waits for the actuator to settle, then de-asserts and holds the signal low — an unpowered servo is never driven. A damper that is merely *holding* a position draws no servo current.
- **OTA / fault safe state.** `INodeHandler::PrepareForReset()` drives the damper to 50% (neutral airflow), then de-asserts `ServoEnable` before resetting (`ControllerNode-Thermostat-Link-Spec.md` §5.1) — `Damper::ParkNeutral()` blocks for the move (~1.5 s, less on a stall) since the reset follows immediately. A lost main-bus connection does not park; the room loop ramps instead (`Damper-Budget-Spec.md` §4.3). A hung or resetting node applies no drive rather than latching its last PWM.
