...
// §5): RESET_NINA + NinaRts GPIOs, USART2 in plain (non-RS485) mode, and the
...
//     Available()/ReadByte()/WriteBytes(), or RawUart() for
//     NodeLib::Frame::Write() -- there are no AT lines to parse any more, so
//     Loop() stops feeding the parser once EnterDataMode() is called.
class NinaAt
{
  public:
    NinaAt();
