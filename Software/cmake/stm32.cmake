...
#       FIXED_NODE_ID  1)                    # optional -- needs MODULE + BOOTLOADER
#
...
# written separately (tools/provision.py), unless FIXED_NODE_ID is given: then
# every unit of this board shares one ConfigRecord (that NodeId + MODULE),
# generated by tools/gen_config_record.py and merged into <name>-full.hex too.
...
function(add_stm32_executable NAME)
    cmake_parse_arguments(ARG "" "LINKER_SCRIPT;BOOTLOADER;MODULE;FIXED_NODE_ID" "SOURCES;LIBRARIES" ${ARGN})

    if(DEFINED ARG_FIXED_NODE_ID AND NOT (ARG_MODULE AND ARG_BOOTLOADER))
        message(FATAL_ERROR "add_stm32_executable(${NAME}): FIXED_NODE_ID needs MODULE and BOOTLOADER")
    endif()
    ...
    if(ARG_BOOTLOADER)
        ...
        set(_full_in ${_boot_hex} ${_hex})
        set(_full_deps ${NAME} ${ARG_BOOTLOADER})

        if(DEFINED ARG_FIXED_NODE_ID)
            # Config A sits above the app, so appending keeps ascending order.
            set(_gen_config ${_STM32_CMAKE_DIR}/../tools/gen_config_record.py)
            set(_config_hex ${CMAKE_CURRENT_BINARY_DIR}/${NAME}-config.hex)
            string(TOLOWER ${ARG_MODULE} _module_name)
            add_custom_command(OUTPUT ${_config_hex}
                COMMAND ${PYTHON3} ${_gen_config}
                    --node-id ${ARG_FIXED_NODE_ID} --module ${_module_name} -o ${_config_hex}
                DEPENDS ${_gen_config}
                VERBATIM
                COMMENT "Fixed ConfigRecord (node ${ARG_FIXED_NODE_ID}, ${ARG_MODULE}) -> ${NAME}-config.hex")
            list(APPEND _full_in ${_config_hex})
            list(APPEND _full_deps ${_config_hex})
        endif()
